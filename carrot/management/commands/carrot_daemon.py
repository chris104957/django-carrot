...
class CarrotService(threading.Thread):
    def __init__(self, **options):
        super(CarrotService, self).__init__()
    ...
    def run(self):
        """
        The main process. Performs the following steps:

        1. Checks to see if the process is already running, and raises a :class:`PidExists` error if so
        2. Writes the pid to the pidfile
        3. Parses the options for the carrot service
        4. Calls the command using :function:`subprocess.Popen`, redirecting the output to :function:`os.devnull`
        ...
        options = ['python3', 'manage.py', 'carrot', '--verbosity', str(self.options['verbosity']),
        ...
        subprocess.Popen(options, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        s = CarrotService(**options)

        if mode == 'start':
            self.stdout.write('Attempting to start the process')
            s.start()
            self.stdout.write(self.style.SUCCESS('Process started successfully'))

        elif mode == 'stop':
            self.stdout.write('Attempting to stop the process')
            s.stop()
            ...
            if s.pid:
                self.stdout.write(self.style.SUCCESS('Service is running. PID: %i' % s.pid))
            else:
